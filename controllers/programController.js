...
exports.getAllProgramsWithCategoryDetails = async (req, res) => {
  try {
    ...
    programs.forEach(p => {
      ...
      if (!grouped[catId]) grouped[catId] = { category: p.category, programs: [] };
...
exports.getCommunityEventPrograms = async (req, res) => {
  try {
    ...
    const programs = await Program.find({ $or: [ { isCommunityEvent: true }, { type: 'community-event' } ] }).populate('category creator');
    res.json({ success: true, data: programs });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
exports.getProgramSurveyAnalytics = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Survey analytics endpoint (chưa triển khai logic)' });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
};